...
SELECT Year, Product, SUM(Sales_Amount) AS SUM_SALES
FROM SALES_DATA
...
SELECT Year, SUM(Sales_Amount) AS SUM_SALESFROM SALES_DATAGROUP BY YearHAVING SUM(Sales_Amount)>500;

--10. Display Regions with at Least 3 Distinct Products Sold.
SELECT Region, COUNT(DISTINCT Product) AS DIS_PRODUCT
FROM SALES_DATA
GROUP BY Region
...
SELECT TOP 1 Region, Year, MIN(Sales_Amount) AS Lowest_Total_Sales
FROM SALES_DATA
WHERE Product='Mobile'
GROUP BY Region, Year
ORDER BY MIN(Sales_Amount);
...
SELECT Year, Region, COUNT(Sales_Amount)
FROM SALES_DATA
...
SELECT TOP 3 Region, SUM(Sales_Amount) AS SUM_SALES
FROM SALES_DATA
WHERE Year='2024'
GROUP BY Region
ORDER BY SUM(Sales_Amount);
...
SELECT TOP 1 Year, SUM(Sales_Amount) AS Lowest_Total_Sales
FROM SALES_DATA
GROUP BY Year
ORDER BY SUM(Sales_Amount);
...
SELECT Region,Year, COUNT(Product) as COUNT_ORDER
FROM SALES_DATA
GROUP BY Region,Year
...
--5. Find out year wise duplicate product.SELECT Year, COUNT(Product) AS COUNT_PRODUCT
FROM SALES_DATA
GROUP BY Year
HAVING COUNT(Product)>COUNT(DISTINCT Product);